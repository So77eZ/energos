...
- **3D-банки → реальные топ-2 напитка (бэк-висяк)** — сейчас банки по бокам каталога (≥1440px) абстрактные. Заменить на 3D топ-2 напитка по среднему рейтингу. Нужно с бэка: поле `can_colors` (body/stripe/accent на напиток) ИЛИ генератор цветов от хеша id + способ взять топ-2. Фронт-часть мелкая (прокинуть цвета/данные в `ThreeCans`). *(Фронт-доработки — гейт ≥1440px без монтирования WebGL ниже + привязка полосок к активному акценту + reduced-motion static — уже в проде, см. archive.)*
